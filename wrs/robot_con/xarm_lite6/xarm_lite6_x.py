...
import numpy as np
from experiments.robot.xarm.utils.motors.dynamixel_gripper import Gripper
...
class XArmLite6X(object):
    def __init__(self, ip: str = "192.168.1.232", reset: bool = False, has_gripper=False):
        ...
        # for grippers
        if has_gripper:
            # self._gripper_x = Gripper(port="/dev/ttyUSB1", dxl_id=2)
            self._gripper_x = Gripper(port=sorted(glob.glob("/dev/ttyUSB*"))[0], dxl_id=2)
    ...
    def homeconf(self):
        self.move_j(jnt_val=np.array([-2.1713  ,  0.44797 ,  1.690495, -3.135073, -1.232058, -0.601129]), )
