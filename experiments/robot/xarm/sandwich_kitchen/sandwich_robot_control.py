...
food_items_cfg = json.load(
    open("experiments/robot/xarm/sandwich_kitchen/food_items.json")
)
...
server.listen(1)
print("[INFO] Robot waiting for sandwich order...")

conn, addr = server.accept()
print(f"[INFO] Connected to order screen: {addr}")
...
def make_sandwich(rbtx, sandwich_order):
    ...
    for item in action_seq:
        ...
        down_pos = np.array(
            [
                item_cfg["preparation_pos"][0],
                item_cfg["preparation_pos"][1],
                0.019,
            ]
        )
        ...
        before_move2table = np.array(
            [
                item_cfg["move2table"][0],
                item_cfg["move2table"][1],
                0.12,
            ]
        )
...
print("[INFO] Ready to accept orders in a loop...")
...
while True:
    ...
    print(f"[INFO] New message received: {data}")

    if data == "SHUTDOWN":
        print("[INFO] Received SHUTDOWN. Closing robot program.")
        conn.close()
        server.close()
        sys.exit(0)

    if data not in sandwich_recipe:
        print(f"[ERROR] Unknown sandwich: {data}")
        continue

    make_sandwich(rbtx, data)
    print("[INFO] Waiting for next order...")
